...
**프로젝트 주제 예시**
1. **학술 논문 자동 분류 시스템**: 연구 논문을 카테고리별로 자동 분류
2. **정책 문서 요약 생성기**: 긴 정책 문서를 핵심 요약문으로 변환
3. **다국어 번역 및 요약**: 외국 문헌 번역 후 핵심 내용 요약
4. **SNS 여론 분석 대시보드**: 트위터, 뉴스 댓글 등의 감성 분석
5. **FAQ 자동 응답 시스템**: 전공 분야 질문에 자동 답변
6. **역사 문헌 개체명 인식**: 역사 문서에서 인물, 장소, 사건 추출
7. **법률 문서 분석기**: 판례, 법령 문서 분석 및 유사 판례 검색
8. **의료 기록 분류 시스템**: 환자 기록 자동 분류 및 요약
9. **금융 뉴스 감성 분석**: 뉴스 기사 기반 시장 심리 분석
10. **소설/시나리오 자동 생성기**: 창작 텍스트 생성
