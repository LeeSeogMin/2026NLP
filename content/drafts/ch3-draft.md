...
1. **고차원 문제**: 어휘가 30,000개면 벡터 차원도 30,000이 된다
2. **의미 관계 부재**: "개"와 "강아지"는 의미가 비슷하지만, 원-핫 벡터 사이의 코사인 유사도는 0이다

이 문제를 해결하기 위해 등장한 것이 **단어 임베딩(Word Embedding)**이다.

### 단어 임베딩: 의미를 담은 벡터

단어 임베딩은 각 단어를 저차원(보통 100~300차원)의 밀집 벡터(Dense Vector)로 표현한다. 핵심 아이디어는 **분포 가설(Distributional Hypothesis)**이다: "비슷한 문맥에 등장하는 단어는 비슷한 의미를 갖는다" (Harris, 1954).

예를 들어, "나는 커피를 마셨다"와 "나는 차를 마셨다"에서 "커피"와 "차"는 같은 문맥에 등장하므로 비슷한 벡터로 표현된다.

### Word2Vec: 임베딩의 혁명

Word2Vec(Mikolov et al., 2013)은 단어 임베딩을 효율적으로 학습하는 방법으로, NLP의 역사를 바꾼 혁신적인 연구이다. 두 가지 학습 방식이 있다:
...
- **CBOW**: 주변 단어(Context)로 중심 단어(Center)를 예측한다. "나는 오늘 ___ 에 갔다"에서 빈칸을 맞추는 문제이다.
- **Skip-gram**: 중심 단어로 주변 단어를 예측한다. "학교"가 주어지면 "나는", "오늘", "에", "갔다"를 맞추는 문제이다.

실제로 Skip-gram 모델을 학습시키면, 의미적으로 유사한 단어들이 임베딩 공간에서 가까이 위치하게 된다:
...
이는 임베딩 공간이 단순히 단어를 나열한 것이 아니라, 성별, 시제, 단복수 같은 **의미적 관계를 기하학적으로 인코딩**하고 있음을 보여준다.

### 사전학습 임베딩

대규모 말뭉치로 미리 학습된 임베딩을 가져다 쓸 수도 있다:

**표 3.1** 주요 사전학습 임베딩 비교

| 모델 | 학습 방식 | 특징 |
|------|----------|------|
| Word2Vec | 예측 기반 (Skip-gram/CBOW) | 로컬 문맥만 사용 |
| GloVe | 통계 기반 (동시출현 행렬) | 전역 통계 활용 |
| FastText | Word2Vec + 서브워드 | 미등록 단어(OOV) 처리 가능 |

FastText는 "unbelievable"을 "un", "believ", "able" 같은 부분 단어로 쪼개어 학습하므로, 학습 데이터에 없는 새로운 단어도 처리할 수 있다는 장점이 있다.
...
여기서 hₜ는 현재 은닉 상태, xₜ는 현재 입력, hₜ₋₁은 이전 은닉 상태이다. 핵심은 **Wₕₕ**가 모든 시점에서 공유된다는 것이다. 이 가중치가 "기억"을 전달하는 역할을 한다.

### 장기 의존성 문제

그러나 RNN에는 근본적인 한계가 있다. 문장이 길어지면 앞부분의 정보가 점점 사라진다. 이를 **장기 의존성 문제(Long-term Dependency Problem)**라 한다.

이유는 역전파 과정에서 기울기가 반복적으로 곱해지기 때문이다. Wₕₕ의 값이 1보다 작으면 기울기가 지수적으로 줄어들고(기울기 소실), 1보다 크면 폭발적으로 커진다(기울기 폭주).

예를 들어 "나는 **프랑스에서** 10년간 살았다. 그래서 나는 ___를 잘한다"에서 빈칸에 "프랑스어"를 넣으려면 멀리 떨어진 "프랑스에서"를 기억해야 한다. RNN은 이런 먼 거리의 의존성을 처리하기 어렵다.
...
- **Forget Gate**: "이 책은 더 이상 필요 없으니 반납하세요" — 이전 정보 중 불필요한 것을 잊는다
- **Input Gate**: "이 새 책은 중요하니 서가에 꽂으세요" — 새 정보 중 중요한 것을 기억한다
- **Output Gate**: "이 책을 빌려가세요" — 현재 필요한 정보만 출력한다

셀 상태(Cell State)는 **컨베이어 벨트**와 같다. 정보가 게이트를 거치며 선택적으로 추가·삭제되지만, 기본적으로 직선 경로를 따라 흘러가므로 기울기가 잘 전달된다.
...
| 파라미터 수 | 더 많음 | 더 적음 |
| 성능 | 대규모 데이터에서 유리 | 소규모 데이터에서 유사 |
| 학습 속도 | 느림 | 빠름 |

### RNN 계열의 근본적 한계

RNN, LSTM, GRU는 모두 중요한 한계를 공유한다:

1. **순차 처리**: 시점 t의 계산은 시점 t-1이 끝나야 시작할 수 있어 **병렬화가 불가능**하다
2. **긴 문맥 처리의 한계**: LSTM도 수백 토큰 이상의 긴 문맥에서는 성능이 저하된다
3. **느린 학습 속도**: 순차 처리로 인해 GPU의 병렬 연산 능력을 활용하지 못한다

이러한 한계를 극복하기 위해 등장한 것이 바로 다음 절에서 다룰 **Attention 메커니즘**이다.
...
문장이 짧으면 괜찮지만, 100단어짜리 문장의 모든 정보를 하나의 벡터에 담는 것은 무리이다.
...
도서관에 비유하면: Query는 "딥러닝 입문서를 찾고 싶다"이고, Key는 각 책의 분류 태그이고, Value는 책의 실제 내용이다. Query와 Key를 비교하여 가장 관련 있는 책(Value)을 찾아온다.
...
**[단계 1] QKᵀ — 유사도 계산**: Query와 모든 Key의 내적을 계산한다. 내적 값이 클수록 두 벡터의 방향이 비슷하다는 뜻이다.

**[단계 2] √dₖ로 나누기 — 스케일링**: Key의 차원(dₖ)이 클수록 내적 값도 커진다. 이러면 softmax 출력이 0 또는 1에 가까워져 기울기가 거의 사라진다. √dₖ로 나누면 분산이 약 1로 안정화된다:
...
**[단계 3] Softmax — 확률 변환**: 스케일링된 점수에 softmax를 적용하여 합이 1인 확률 분포(가중치)로 변환한다.

**[단계 4] 가중합 — 최종 출력**: 각 Value에 대응하는 가중치를 곱하고 합산한다. 가중치가 높은 단어의 정보가 더 많이 반영된다.
...
각 행은 해당 단어가 다른 단어에 얼마나 "주목"하는지를 나타낸다. "은행에서"는 자기 자신(0.768)에 가장 많이 주목하고, "나는"(0.140)에도 일부 주목한다.
...
앞서 본 Attention은 Encoder-Decoder 사이에서 사용되었다. **Self-Attention**은 같은 문장 안에서 단어들이 서로를 참조하는 것이다. Q, K, V가 모두 **같은 입력**에서 생성된다는 것이 핵심 차이이다.
...
이처럼 Self-Attention은 **문맥에 따라 단어의 의미를 동적으로 결정**한다.
...
Self-Attention 하나로는 한 가지 관점만 포착한다. 그러나 언어에는 다양한 관계가 있다. "그녀가 말한 것을 그는 이해했다"에서:
...
1. d_model 차원을 num_heads개로 분할한다. d_model=32, num_heads=4이면 각 Head는 d_k=8 차원에서 독립적으로 Attention을 수행한다.
2. 각 Head의 출력을 이어붙인다(Concatenation).
3. 최종 Linear 층(W_O)으로 변환하여 원래 차원(d_model)으로 복원한다.
...
각 Head가 실제로 다른 패턴을 포착하는지 확인해보면:
...
각 Head가 다른 단어에 주목하고 있음을 확인할 수 있다.
...
Scaled Dot-Product Attention을 단계별로 구현한다:
...
핵심은 d_model 차원을 num_heads로 분할하여 각 Head가 독립적으로 Attention을 수행하는 것이다:
...
학습된 Attention Weight를 히트맵으로 시각화하면 모델이 어떤 단어 쌍에 주목하는지 직관적으로 파악할 수 있다:

_시각화 결과는 practice/chapter3/data/output/attention_heatmap_ko.png 참고_

### Attention 기반 감성 분류 모델

Self-Attention을 활용하여 간단한 감성 분류 모델을 구현한다. 임베딩 → Self-Attention → 평균 풀링 → 분류기 구조이다:
...
- **Self-Attention**은 같은 문장 안의 단어들이 서로를 참조하여 문맥에 따른 의미를 결정한다
- **Multi-Head Attention**은 여러 관점에서 동시에 Attention을 수행하여 다양한 관계를 포착한다
...
2. Pennington, J. et al. (2014). GloVe: Global Vectors for Word Representation. *EMNLP*. https://nlp.stanford.edu/pubs/glove.pdf
3. Hochreiter, S. & Schmidhuber, J. (1997). Long Short-Term Memory. *Neural Computation*, 9(8), 1735-1780.
4. Cho, K. et al. (2014). Learning Phrase Representations using RNN Encoder-Decoder. *EMNLP*. https://arxiv.org/abs/1406.1078
5. Bahdanau, D. et al. (2014). Neural Machine Translation by Jointly Learning to Align and Translate. *ICLR*. https://arxiv.org/abs/1409.0473
6. Vaswani, A. et al. (2017). Attention Is All You Need. *NeurIPS*. https://arxiv.org/abs/1706.03762
