...
**강화할 점**:
- 청킹의 중요성: RAG 성능의 첫 번째 결정 요소는 문서 분할 방식이다. 너무 작으면 맥락 부족, 너무 크면 검색 부정확이다.
- Hybrid 검색의 가치: Dense 검색만으로는 키워드를 정확히 찾지 못하고, Sparse 검색만으로는 의미적 관련성을 놓친다. 둘을 결합하면 훨씬 강건하다.
- 프롬프트 엔지니어링: "컨텍스트에만 기반하여 답변하세요" 같은 명확한 지시사항이 LLM의 환각을 줄인다.

**주의할 점**:
- 벡터 DB의 크기 제한: FAISS는 메모리 기반이므로 대규모 문서(100만 개 이상)에는 적합하지 않다. 프로덕션에서는 Pinecone, Weaviate 등 클라우드 솔루션을 고려한다.
- 임베딩 모델의 한계: 임베딩 모델이 "정답"을 보장하는 것이 아니라, 의미적 유사도만 계산할 뿐이다. 실제 관련성은 재랭킹 단계에서 개선된다.
- LLM의 토큰 제한: 검색된 문서가 너무 많으면 LLM의 입력 길이를 초과할 수 있다. K 값 조정과 컨텍스트 요약이 필요하다.

**다음 학습으로의 연결**:
- 11주차 C회차(모범 구현): 더 정교한 청킹 기법(Semantic Chunking), Reranker 모델(Cross-Encoder) 적용, Multi-Turn RAG 구현
- 12주차: Fine-tuning 기반 접근법과 RAG의 조합 (Hybrid Fine-tuning + RAG)
- 최종 프로젝트: 실제 회사 문서 또는 논문 기반 RAG 시스템 구축 및 배포
