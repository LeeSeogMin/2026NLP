...
**강화할 점**:
- **메모리의 계층적 이해**: 파라미터, 옵티마이저, 그래디언트, 활성화 등 각 요소의 메모리 사용을 명확히 하고, 어디를 줄일 수 있는지 체계적으로 접근
- **LoRA의 통찰**: 파라미터 변화가 저랭크 구조를 가진다는 경험적 관찰이 얼마나 강력한지 강조 (이론과 실제의 정렬)
- **4-bit 양자화의 견고성**: 정보 손실이 있음에도 모델 성능이 유지되는 이유를 신경망의 특성 관점에서 설명
- **target_modules 선택의 중요성**: Query와 Value만으로도 충분한 성능을 얻을 수 있다는 것은 Attention의 구조에 대한 깊은 이해에서 비롯됨

**주의할 점**:
- **LoRA의 한계**: "LoRA = 완벽한 Full FT 대체"라는 착각 방지. 극도로 다른 도메인이나 새로운 언어에서는 성능 격차가 발생할 수 있음
- **메모리 계산의 함정**: Double Quantization이 항상 필수는 아니며, 상황에 따라 일반 int4가 충분할 수 있음
- **배치 크기의 관계**: 배치 1 + 그래디언트 누적은 배치 여러 개와 다르며, 배치 정규화(Batch Norm) 층이 있을 때는 영향을 고려해야 함

**다음 학습으로의 연결**:
- 11주차: "생성형 LLM 파인튜닝 (1) — Instruction Tuning과 RLHF"에서 더 큰 모델(13B, 70B)을 다루며, QLoRA의 실전 활용을 심화한다
- 12주차: "벡터 DB와 RAG: 파인튜닝 없이 지식 주입하기"에서 메모리 제약이 더 심한 상황(엣지 디바이스, 온라인 서버)에서의 추론 최적화를 다룬다
- **통합 관점**: 파인튜닝 효율화(LoRA) + 추론 최적화(양자화) + 지식 주입(RAG)이 함께 작동하여 제한된 리소스에서도 강력한 LLM 시스템을 구축할 수 있음을 보여준다
