...
**강화할 점**:
- **Tool의 역할**: Tool은 단순 함수 호출이 아니라, Agent가 "문제 해결을 위해 자동으로 선택하는 능력"의 핵심이다. 좋은 Tool 설명은 LLM이 Tool을 올바르게 사용하도록 돕는다.
- **ReAct의 투명성**: THOUGHT-ACTION-OBSERVATION이 명확하게 기록되면, Agent의 사고 과정을 이해하고 오류를 디버깅할 수 있다.
- **Tool Composition의 유연성**: 복잡한 문제도 여러 도구의 조합으로 해결 가능함을 보여준다. 이는 13장 상황 인식 Agent로 확장된다.

**주의할 점**:
- **Hallucination 위험**: Tool이 없으면 LLM은 사실이 아닌 정보를 생성할 수 있다. Tool과 LLM의 조합은 정확성을 크게 개선한다.
- **Tool 실행 오류**: 모든 Tool이 항상 성공하지는 않는다. 네트워크 끊김, DB 오류, 입력 오류 등 다양한 실패 시나리오에 대비해야 한다.
- **무한 반복 방지**: max_iterations 제한은 필수다. 그렇지 않으면 Agent가 같은 도구를 반복 호출할 수 있다.

**다음 학습으로의 연결**:
- 12주차 B회차는 기본 ReAct Agent를 구현한다.
- 13주차 A회차에서는 **상황 인식 Agent(Situation Aware Agent)**를 다룬다. 더 많은 도구, 더 복잡한 문제 상황, 사전 정보(context) 활용 등을 포함한다.
- 13주차 B회차에서는 실제 비즈니스 시나리오(예: 고객 지원 Agent, 데이터 분석 Agent)에 LangGraph Agent를 적용한다.
