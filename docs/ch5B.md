...
**강화할 점**:
- BERT와 GPT를 "이해" vs "생성"의 대척점으로 이해하되, 실무에서는 두 모델을 보완적으로 활용한다는 점을 강조
- 개체명 인식이 정확해야 생성 결과가 좋아진다는 **파이프라인의 연쇄 효과**를 강조
- 디코딩 전략의 선택이 모델 선택만큼 중요하다는 점 — "같은 모델도 전략에 따라 완전히 다른 결과"
- Transfer Learning의 강력함 — 사전학습된 모델 사용으로 며칠의 학습이 수시간으로 단축되는 실무적 이점
- Hugging Face가 얼마나 효율적인 도구인지 — 수백 줄의 복잡한 구현을 3~10줄로 축약

**주의할 점**:
- NER 결과가 완벽하지 않다는 것을 수용하고, "완벽한 추출"이 아니라 "실용적인 수준의 정확도"를 목표로 삼아야 함을 강조
- 생성된 텍스트의 품질이 학습 데이터(GPT-2는 WebText, 2019년)의 특성을 반영한다는 점 — 최신 정보나 특정 도메인 지식은 부족할 수 있음
- "생성 ≠ 사실" — 모델이 유창한 텍스트를 생성해도 사실성을 보장하지 않음. 특히 환각(Hallucination) 현상이 발생할 수 있음
- 프롬프트 품질에 따라 생성 결과가 급격히 달라진다는 점 — 프롬프트 엔지니어링이 실무에서 중요한 스킬

**다음 학습으로의 연결**:
- 6장 "Fine-tuning과 평가": 이번 실습에서 사용한 사전학습 모델을 특정 도메인에 미세조정하는 방법
- 7장 "고급 기법 (Prompt Engineering, In-Context Learning)": GPT-3/4처럼 프롬프트만으로 다양한 과제를 수행하는 기법
- 8장 "배포와 실무": 이번 NER+생성 파이프라인을 실제 서비스로 배포할 때 고려할 사항 (속도, 비용, 안정성)

**학생들에게 던질 질문**:
1. "왜 같은 BERT 모델도 다양한 용도(감성분석, NER, QA)로 활용될 수 있을까?" → Transfer Learning과 Fine-tuning의 강점
2. "Greedy로 생성한 텍스트가 가장 안정적인데, 왜 Top-p나 Temperature를 사용할까?" → 다양성과 창의성의 가치
3. "BERT NER 모델이 특정 개체를 놓쳤다면, 미세조정으로 개선할 수 있을까?" → 다음 장으로의 연결
