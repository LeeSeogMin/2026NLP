...
**강화할 점**:
- Bag-of-Words의 단순함과 한계를 명시: 단어 순서를 무시하므로 "좋다 싫다"와 "싫다 좋다"를 같게 본다
- 은닉층이 입력을 "중간 표현"으로 변환한다는 개념을 강조: 첫 은닉층이 낮은 수준의 패턴(단어 조합), 두 번째 은닉층이 높은 수준의 패턴(감정)을 포착
- Dropout이 과적합을 방지하는 메커니즘: "뉴런을 랜덤으로 끄기"는 결국 **앙상블**과 같은 효과
- Precision과 Recall의 트레이드오프 이해: 분류 기준을 엄격히 하면 Precision ↑, Recall ↓

**주의할 점**:
- 정확도만으로는 불충분: 클래스 불균형이 심하면 정확도는 높지만 소수 클래스 성능이 나쁠 수 있음
- 하이퍼파라미터 "최적값"은 데이터에 따라 다름: 이 과제의 최고 정확도가 다른 데이터셋에서도 최적은 아닐 수 있음
- Bag-of-Words의 희소성: 10,000차원 벡터의 대부분이 0이므로 계산 효율이 나쁨 (3주차 임베딩이 해결)

**다음 학습으로의 연결**:
- 3주차에서는 단어 순서를 고려한 **임베딩과 RNN** → **Self-Attention**으로 진화
- Attention Weight 시각화를 통해 모델이 "어디에 집중하는가"를 해석
- 4주차 **Transformer**는 RNN 없이 Attention만으로 문장의 모든 단어를 병렬 처리하여 훨씬 빠르고 강력함
- 이 MLP 기반의 감성 분류 기술은 4주차 Transformer 텍스트 분류로 직결되며, 최종적으로 5주차 BERT/GPT 활용까지 연결됨
