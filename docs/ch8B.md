...
| 01:20~01:25 | 체크포인트 4: CLIP 멀티모달 시연                                   | 보조용으로 가능  |
...
### 8.8 멀티모달 토픽 모델링 — 텍스트 + 이미지

> **데이터·코드 모두 준비되어 있다**. `practice/chapter8/data/input/ch8B_multimodal_data.csv` (20개 패션 상품 캡션) + `practice/chapter8/data/input/multimodal_images/` (대응 PNG 20장)을 노트북 §15에서 자동 로드한다. 첫 실행 시 CLIP 모델(약 600MB)이 다운로드되며, `bertopic[vision]`이 미설치이거나 다운로드를 피하고 싶다면 셀의 `RUN_MULTIMODAL`을 `False`로 둔다.
...
#### 8.8.3 실습 데이터 — 패션 상품 20개

`ch8B_multimodal_data.csv`는 4개 카테고리(드레스 5 / 재킷·아우터 5 / 신발 5 / 가방 5)로 구성된 20개 패션 상품의 영문 캡션과 PNG 이미지 쌍이다. 작지만 카테고리 분리가 명확해 CLIP의 효과를 한눈에 보여주는 학습용 데이터셋이다.

**기대 결과**: 텍스트만으로 분석하면 "leather"라는 단어가 재킷·신발·가방에 모두 등장해 토픽이 흐려지지만, 이미지를 결합하면 시각적 형태(평면 vs 굽 vs 가방 모양)가 벡터 공간에 반영되어 카테고리별로 깔끔하게 분리된다.
...
#### 체크포인트 4 (15분) — CLIP 멀티모달 시연

데이터·코드 모두 준비되어 있다(`data/input/ch8B_multimodal_data.csv` + `data/input/multimodal_images/`). 첫 실행 시 약 600MB의 CLIP 모델이 다운로드된다.

**수행 순서**:

1. 노트북 §15 셀의 `RUN_MULTIMODAL = True` 확인 (기본값)
2. `bertopic[vision]` 미설치라면 `pip install "bertopic[vision]" pillow` 실행
3. 셀을 실행하면 CSV·이미지가 자동 로드되고 CLIP 임베딩으로 BERTopic 학습
4. 토픽별 키워드와 항목 수가 출력되며 `data/output/ch8B_multimodal_topic_info.csv`에 저장된다

**산출물**:

- `data/output/ch8B_multimodal_topic_info.csv` (토픽별 키워드·대표 캡션)

**리포트 (1문단)**: 카테고리(드레스/재킷/신발/가방)가 토픽으로 얼마나 잘 분리되었는가? 캡션만 보고 묶었을 때 혼동될 단어("leather", "black" 등)가 이미지 결합 후에도 문제를 일으켰는가?
