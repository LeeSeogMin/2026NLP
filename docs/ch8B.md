...
**강화할 점**:
- BERTopic이 자동으로 주제를 발견한다는 것의 의미: 데이터 기반의 자동화의 가치
- 각 주제의 상위 단어가 "정말 그 주제를 잘 대표하는가"를 비판적으로 평가하는 능력
- BERT 임베딩의 의미론적 이해가 토픽 모델링 성능에 직결됨을 재강조
- 토픽 코히어런스 같은 정량적 평가 지표의 중요성

**주의할 점**:
- "높은 코히어런스 점수 = 좋은 토픽 모델링"이 아님: 맥락과 목표에 따라 달라질 수 있음
- 자동으로 발견된 주제도 완벽하지 않을 수 있음: 경계 문서, 중복 주제 등의 한계
- 문서 전처리(토큰화, 불용어 제거)가 결과에 영향을 미칠 수 있음을 언급
- "주제 개수가 많을수록 좋다"는 오해 방지: 과다분할(Over-fragmentation)의 위험성

**다음 학습으로의 연결**:
- 9주차: LDA와 BERTopic을 직접 비교 실행하고 성능 차이 분석
- 10주차: 동적 토픽 모델링(Dynamic Topic Modeling) — 시간에 따른 주제 변화 추적
- 11~12주차: 프로젝트 단계에서 토픽 모델링을 실제 데이터에 적용
