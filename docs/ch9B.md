...
**강화할 점**:
- Full Fine-tuning은 "모든 파라미터 업데이트"라는 원칙이 데이터 제약 상황에서 과적합으로 이어지기 쉬움을 명시
- Early Stopping의 효과: Val Loss가 최소일 때 모델을 저장함으로써 과적합을 조기에 방지할 수 있음
- Confusion Matrix는 단순히 "틀린 개수"가 아니라 **어떤 종류의 오류**인지 분석하는 도구임을 강조
- 클래스 불균형 없이도 과적합이 발생할 수 있음: 작은 데이터셋(45K 훈련 샘플) + 큰 모델(110M 파라미터)

**주의할 점**:
- "검증 정확도 82%면 좋은가"라는 질문에 대해: YNAT는 9개 카테고리이므로 무작위 추측은 11%이다. 82%는 좋은 성능이지만, 실제 용도를 고려해야 한다 (뉴스 카테고리 분류는 오류 비용이 낮음)
- 학습률 2e-5의 "마법성" 방지: 이는 경험적 가이드일 뿐, 데이터와 모델에 따라 조정이 필요하다
- Trainer의 자동 관리 편의성: 계산 세부사항은 숨겨졌지만, 무엇이 일어나는지 이해하는 것이 중요 (다음 주 PEFT에서는 개별 요소를 직접 조작해야 함)

**다음 학습으로의 연결**:
- Full Fine-tuning의 문제점: 메모리 사용(GPU 메모리 초과), 속도(학습 시간 길어짐), 개인 환경에서의 어려움
- 10주차에서 배울 PEFT (특히 LoRA): 0.1% 파라미터만 학습하면서도 Full Fine-tuning과 유사한 성능을 얻는 기법
- "같은 결과를 더 빠르고 저렴하게" 얻을 수 있는 혁신적 접근
