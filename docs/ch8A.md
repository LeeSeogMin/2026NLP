...
7가지 내장 시각화는 정량 분석용이다. 발표·보고서용 인포그래픽이 필요할 때는 외부 `wordcloud` 패키지로 **토픽별 워드클라우드**를 만들 수 있다. BERTopic에는 의존성 최소화 정책상 **내장되지 않으며**, 공식 Tips & Tricks(Grootendorst)가 다음 패턴을 권장한다.

```python
from wordcloud import WordCloud
import matplotlib.pyplot as plt

def topic_wordcloud(model, topic_id, font_path):
    freq = {w: s for w, s in model.get_topic(topic_id)}  # 단어 → c-TF-IDF 점수
    wc = WordCloud(background_color="white", max_words=50,
                   font_path=font_path).generate_from_frequencies(freq)
    plt.imshow(wc, interpolation="bilinear"); plt.axis("off"); plt.show()
```

> **한국어 주의**: `font_path`에 한글 폰트 경로(예: `"/System/Library/Fonts/AppleSDGothicNeo.ttc"`, 나눔고딕 등)를 반드시 지정해야 한다. 미지정 시 한글이 □로 깨진다. 단어 크기는 `WordCloud`가 c-TF-IDF 점수에 비례해 자동 결정하므로, 일반 빈도 워드클라우드보다 토픽 고유어가 더 또렷하게 부각된다.

> **두 층위 병행 권장**:
> - `visualize_topics()` — 항목 = **토픽**, 원 크기 = 토픽 비중 → **전체 코퍼스의 토픽 분포** (워드클라우드의 본질적 대응)
> - 토픽별 워드클라우드 — 항목 = **단어**, 크기 = c-TF-IDF → **한 토픽 내부의 키워드 분포** (토픽당 1장)
>
> 보고서에서 두 층위를 함께 제시하면 "어떤 토픽들이 / 무엇으로" 구성되는지 한눈에 보인다.
