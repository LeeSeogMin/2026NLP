...
| 00:55~01:25 | 라이브 코딩 시연 (nn.Module MLP 정의 + 역전파 + 평가) | 교수 시연 시 선택 사용 |
...
**__init__**: 모델의 구조를 정의한다. 각 층이 자신의 파라미터(w, b)를 가진다. nn.Linear(10, 32)는 10차원 입력을 32차원 출력으로 변환한다 — 10×32 + 32 = 352개 파라미터.
...
> **라이브 코딩 시연**: 교수가 `nn.Module`을 상속받아 간단한 2층 MLP를 정의하고, 손실 함수와 옵티마이저를 설정한 뒤, 한 번의 역전파 과정을 실시간으로 추적하며, 모델을 평가하는 전체 과정을 보여준다.
...
"단층 퍼셉트론은 _____________________ 때문에 XOR 문제를 풀 수 없지만, 은닉층을 추가한 MLP는 _____________________를 통해 이 문제를 해결할 수 있다."
