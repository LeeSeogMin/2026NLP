...
| 00:55~01:25 | 라이브 코딩 시연 (Encoder Block 구성요소 구현) | 교수 시연 시 선택 사용 |
...
**직관적 이해**: Transformer는 "동시통역 시스템"과 같다. RNN 기반 모델은 한 명의 통역사가 원문을 한 글자씩 순서대로 읽으며 동시에 해석하는 방식이다. 반면 Transformer는 여러 통역사(Multi-Head Attention)가 문장 전체를 동시에 훑어보고, 각자 다른 부분에 집중하여 분석하는 방식이다.
...
> **쉽게 말해서**: RNN은 "버킷 릴레이"(한 명씩 물을 옆으로 넘기기)이고, Transformer는 "공중 방송"(한 번에 모두에게 정보 전달)이라는 뜻이다.
...
**직관적 이해**: 당신이 대형 마트의 상품을 정렬하려고 한다. 상품 자체는 "맥주", "우유" 같이 이름이 있지만(임베딩), 진열 순서도 중요하다("우유는 1번 진열대", "맥주는 5번 진열대"). 상품 정보에 위치 정보를 덧붙여야 정확히 관리할 수 있다. 이것이 바로 Positional Encoding이다.
...
3. **상대적 위치 표현**: PE(pos+k)를 PE(pos)의 선형 함수로 표현할 수 있어, 모델이 절대 위치보다 **상대적 위치 관계**를 학습하기 쉽다. "위치 5는 위치 3보다 2칸 뒤다"라는 정보가 자동으로 인코딩된다.
...
> **쉽게 말해서**: Sinusoidal PE는 "음악의 화음"처럼 작동한다. 여러 주파수를 섞으면 복잡한 음향이 나오는데, 이 복합 신호가 각 위치를 고유하게 표현한다.
...
**직관적 이해**: 새로운 프로그래밍 언어를 배울 때, 기존에 알고 있는 Python을 완전히 잊고 처음부터 배우지 않는다. 기존 지식(원본)을 유지하면서 새로운 것(변화)만 추가한다. **Residual Connection**도 같은 원리이다.
...
1. **깊은 네트워크에서 기울기가 잘 전달된다**: 역전파 시 기울기가 "고속도로"를 따라 원본 입력까지 직접 흐를 수 있다.
...
**직관적 이해**: 여러 센서(차원)에서 들어오는 신호의 크기가 들쭉날쭉하면 시스템이 불안정해진다. 예를 들어 온도 센서는 0-100℃, 습도 센서는 0-100%, 기압 센서는 950-1050 hPa 범위의 값을 낸다. 이들을 그대로 사용하면 기압 신호가 온도와 습도를 압도한다. Layer Normalization은 모든 센서의 신호를 표준화하여(평균 0, 표준편차 1), 각 센서가 균형 있게 기여하도록 한다.
...
> **쉽게 말해서**: Layer Normalization은 각 은닉 벡터의 "음량"을 조정하여 학습을 더 안정적으로 만드는 기법이다. 마치 오디오 시스템의 게인(gain) 조정처럼.
...
**FFN의 역할**: Self-Attention은 "어떤 단어들 사이에 관계가 있는가"를 찾는다. 반면 FFN은 "각 위치에서 관계를 바탕으로 어떤 변환을 할 것인가"를 수행한다. 예를 들어 Attention이 "형용사와 명사가 관련있다"를 찾으면, FFN은 "그렇다면 형용사의 의미를 명사에 반영하자"는 식의 변환을 한다.
...
**직관적 이해**: 시험을 보며 답안을 쓴다고 생각해 보자. 1번 문제 답을 쓸 때, 2~100번 문제 내용을 미리 볼 수 있다면? 부정행위다. 각 문제를 풀 때는 그 문제까지만 봐야 한다. **Causal Masking**도 같은 원리이다 — 각 위치는 자신과 **이전 위치만** 참조할 수 있고, 미래 위치는 "커닝" 할 수 없다.
...
**직관적 이해**: 번역 작업을 생각해 보자. 한국어 문장을 영어로 번역할 때, 각 영어 단어를 생성할 때마다 원문 한국어를 참고한다. 예를 들어 "I"를 생성할 때는 "나는"을 보고, "went"를 생성할 때는 "갔다"를 본다. **Cross-Attention**은 이렇게 각 생성 단계에서 원문(Encoder 출력)의 관련 부분을 동적으로 찾아본다.
...
> **쉽게 말해서**: 번역할 때 계속 원문을 참고하는 것처럼, Decoder의 각 단계에서 Encoder의 출력을 참고한다.
...
**직관적 이해**: "unbelievable"이라는 단어를 통째로 하나의 토큰으로 처리하면 어휘 사전이 폭발적으로 커진다. 영어만 해도 수십만 단어가 있고, 신조어와 고유명사까지 합치면 끝이 없다.

반대로 글자 단위로 쪼개면? 어휘 사전은 작지만(알파벳 26개), 하나의 단어를 표현하는 데 토큰이 너무 많이 필요하다.
...
> **쉽게 말해서**: BERT의 토크나이저는 "음절 단위"로 끊고, GPT-2의 토크나이저는 "단어 단위"를 선호한다는 뜻이다.
...
> **라이브 코딩 시연**: 교수가 Transformer Encoder Block을 PyTorch로 한 단계씩 구현하며, Residual Connection과 Layer Normalization의 역할을 실시간으로 보여준다.
